...
package com.example.androidannotationtesttwo.initview;
...
import com.example.androidannotationtesttwo.widget.slidingmenu.SlidingMenu.CanvasTransformer;
...
public class SlidingMenuView {
    ...
    public SlidingMenu initSlidingMenuView( Activity activity, View view) {
        ...
        slidingMenu.setTouchModeAbove(SlidingMenu.SLIDING_WINDOW);// 设置要使菜单滑动，触碰屏幕的范围 SLIDING_CONTENT整个都可以滑动
        // slidingMenuView.setTouchModeBehind(SlidingMenu.SLIDING_CONTENT);//设置了这个会获取不到菜单里面的焦点，所以先注释掉
        slidingMenu.setShadowWidthRes(R.dimen.shadow_width);// 设置阴影图片的宽度 shadow_width
        slidingMenu.setShadowDrawable(R.drawable.shadow);// 设置阴影图片
        slidingMenu.setBehindOffsetRes(R.dimen.slidingmenu_offset);// SlidingMenu划出时主页面显示的剩余宽度 slidingmenu_offset
        slidingMenu.setFadeDegree(0.85F);// SlidingMenu滑动时的渐变程度  设置改变的时候没有明显的效果
